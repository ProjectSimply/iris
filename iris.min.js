...
* Fork of https://github.com/Automattic/Iris
* https://github.com/projectsimply/Iris
* stops the autocomplete on hex codes that causes frustration
...
el.remove()}}
function createGradient(origin,stops){origin=(origin==='top')?'top':'left';stops=Array.isArray(stops)?stops:Array.prototype.slice.call(arguments,1);if(gradientType==='webkit'){return legacyWebkitGradient(origin,stops)}else{return vendorPrefixes[gradientType]+'linear-gradient('+origin+', '+stops.join(', ')+')'}}
function stupidIEGradient(origin,stops){var type,self,lastIndex,filter,startPosProp,endPosProp,dimensionProp,template,html;origin=(origin==='top')?'top':'left';stops=Array.isArray(stops)?stops:Array.prototype.slice.call(arguments,1);type=(origin==='top')?0:1;self=$(this);lastIndex=stops.length-1;filter='filter';startPosProp=(type===1)?'left':'top';endPosProp=(type===1)?'right':'bottom';dimensionProp=(type===1)?'height':'width';template='<div class="iris-ie-gradient-shim" style="position:absolute;'+dimensionProp+':100%;'+startPosProp+':%start%;'+endPosProp+':%end%;'+filter+':%filter%;" data-color:"%color%"></div>';html='';if(self.css('position')==='static'){self.css({position:'relative'})}
...
$.fn.gradient=function(){var args=arguments;return this.each(function(){if(nonGradientIE){stupidIEGradient.apply(this,args)}else{$(this).css('backgroundImage',createGradient.apply(this,args))}})};$.fn.rainbowGradient=function(origin,args){var opts,template,i,steps;origin=origin||'top';opts=$.extend({},{s:100,l:50},args);template='hsl(%h%,'+opts.s+'%,'+opts.l+'%)';i=0;steps=[];while(i<=360){steps.push(template.replace('%h%',i));i+=30}
return this.each(function(){$(this).gradient(origin,steps)})};Iris={options:{color:!1,mode:'hsl',controls:{horiz:'s',vert:'l',strip:'h'},hide:!0,border:!0,target:!1,width:200,palettes:!1,type:'full',slider:'horizontal'},_color:'',_palettes:['#000','#fff','#d33','#d93','#ee2','#81d742','#1e73be','#8224e3'],_inited:!1,_defaultHSLControls:{horiz:'s',vert:'l',strip:'h'},_defaultHSVControls:{horiz:'h',vert:'v',strip:'s'},_scale:{h:360,s:100,l:100,v:100},_create:function(){var self=this,el=self.element,color=self.options.color||el.val();if(gradientType===!1){testGradientType()}
...
self.onlySlider=self.options.type==='hue';self.horizontalSlider=self.onlySlider&&self.options.slider==='horizontal';if(self.onlySlider){self.options.controls.strip='h';if(!color){color='hsl(10,100,50)'}}
...
if(self.options.border&&!self.onlySlider){self.picker.addClass('iris-border')}
self._initControls();self.active='external';self._dimensions();self._change()},_has:function(needle,haystack){var ret=!1;$.each(haystack,function(i,v){if(needle===v){ret=!0;return!1}});return ret},_addPalettes:function(){var container=$('<div class="iris-palette-container" />'),palette=$('<a class="iris-palette" tabindex="0" />'),colors=Array.isArray(this.options.palettes)?this.options.palettes:this._palettes;if(this.picker.find('.iris-palette-container').length){container=this.picker.find('.iris-palette-container').detach().html('')}
$.each(colors,function(index,val){palette.clone().data('color',val).css('backgroundColor',val).appendTo(container).height(10).width(10)});this.picker.append(container)},_paint:function(){var self=this;if(self.horizontalSlider){self._paintDimension('left','strip')}else{self._paintDimension('top','strip')}
self._paintDimension('top','vert');self._paintDimension('left','horiz')},_paintDimension:function(origin,control){var self=this,c=self._color,mode=self.options.mode,color=self._getHSpaceColor(),target=self.controls[control],controlOpts=self.options.controls,stops;if(control===self.active||(self.active==='square'&&control!=='strip')){return}
...
target.gradient(origin,stops);break;default:break}},_getHSpaceColor:function(){return(this.options.mode==='hsv')?this._color.toHsv():this._color.toHsl()},_stripOnlyDimensions:function(){var self=this,width=this.options.width,height=width*0.12;if(self.horizontalSlider){self.picker.css({width:width,height:height}).addClass('iris-only-strip iris-strip-horiz')}else{self.picker.css({width:height,height:width}).addClass('iris-only-strip iris-strip-vert')}},_dimensions:function(reset){if(this.options.type==='hue'){return this._stripOnlyDimensions()}
var self=this,opts=self.options,controls=self.controls,square=controls.square,strip=self.picker.find('.iris-strip'),squareWidth='77.5%',stripWidth='12%',totalPadding=20,innerWidth=opts.border?opts.width-totalPadding:opts.width,controlsHeight,paletteCount=Array.isArray(opts.palettes)?opts.palettes.length:self._palettes.length,paletteMargin,paletteWidth,paletteContainerWidth;if(reset){square.css('width','');strip.css('width','');self.picker.css({width:'',height:''})}
...
position.left=Math.max(0,Math.min(position.left,container.width()));position.top=Math.max(0,Math.min(position.top,container.height()));draggable.css(position);self._squareDrag(event,{position:position});event.preventDefault()});square.on('mousedown',function(event){var squareOffset,pos;if(event.which!==1){return}
...
event.stopPropagation();$(this).trigger('click')})},_squareDrag:function(event,ui){var self=this,controlOpts=self.options.controls,dimensions=self._squareDimensions(),vertVal=Math.round((dimensions.h-ui.position.top)/dimensions.h*self._scale[controlOpts.vert]),horizVal=self._scale[controlOpts.horiz]-Math.round((dimensions.w-ui.position.left)/dimensions.w*self._scale[controlOpts.horiz]);self._color[controlOpts.horiz](horizVal)[controlOpts.vert](vertVal);self.active='square';self._change.apply(self,arguments)},_setOption:function(key,value){var self=this,oldValue=self.options[key],doDimensions=!1,hexLessColor,newColor,method;self.options[key]=value;switch(key){case 'color':if(self.onlySlider){value=parseInt(value,10);if(isNaN(value)||value<0||value>359){value=oldValue}else{value='hsl('+value+',100,50)'}
self.options.color=self.options[key]=value;self._color=new Color(value).setHSpace(self.options.mode);self.active='external';self._change()}else{value=''+value;hexLessColor=value.replace(/^#/,'');newColor=new Color(value).setHSpace(self.options.mode);if(newColor.error){self.options[key]=oldValue}else{self._color=newColor;self.options.color=self.options[key]=self._color.toString();self.active='external';self._change()}}
...
if(self.element.is(':input')&&!self._color.error){self.element.removeClass('iris-error');if(self.onlySlider){if(self.element.val()!==self.hue){self.element.val(self.hue)}}else{if(self.element.val()!==self._color.toString()){self.element.val(self._color.toString())}}}
...
(function(global,undef){var Color=function(color,type){if(!(this instanceof Color))
...
return this._error();this.error=!1;return this.fromInt(parseInt((rgb.r<<16)+(rgb.g<<8)+rgb.b,10),preserve)},fromHex:function(color){color=color.replace(/^#/,'').replace(/^0x/,'');this.error=!/^[0-9A-F]{6}$/i.test(color);return this.fromInt(parseInt(color,16))},fromHsl:function(hsl){var r,g,b,q,p,h,s,l;if(typeof hsl!=='object'||hsl.h===undef||hsl.s===undef||hsl.l===undef)
